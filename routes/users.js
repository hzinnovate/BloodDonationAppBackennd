...
router.post('/register', (req, res) => {
    ...
    const newUser = new Users({email: user.email, password: hash, name: user.name, bloodgroup: user.bloodgroup});
...
router.post('/login', async (req, res) => {
    ...
    if(!passwordMatched) {
        res.send(401, {message: "Incorrect Email/Password!"});
        return;
    }

    //Generate Token
    const token = jwt.sign({user: user[0]}, 'hasnain95');
    user.token
    res.send({user});
})
